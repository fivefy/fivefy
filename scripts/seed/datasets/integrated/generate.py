...
def generate_all(config: ScaleConfig, output_dir: Path) -> None:
    ...
    write_csv(output_dir / "track_applications.csv", ["id","requester_user_id","track_type","artist_id","album_id","track_number","title","lyrics","genre","audio_key","duration_sec","featured_artist_text","publish_delay_days","status","reviewed_by_admin_id","reviewed_at","rejection_reason","created_at","updated_at"], generate_track_applications(config.track_applications, config.users, config.artists, config.albums, album_artist_ids))
