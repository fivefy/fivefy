...
TABLE_SPECS: list[dict[str, Any]] = [
    ...
    {"table":"payments","file":"payments.csv","columns":["id","cash_order_id","user_id","amount","status","order_number","pg_transaction_id","webhook_id","refund_reason","paid_at","refunded_at","created_at"],"nullable_columns":{"refund_reason","paid_at","refunded_at"}},
    {"table":"subscriptions","file":"subscriptions.csv","columns":["id","point_order_id","plan_type","status","start_date","expiry_date","next_billing_date","created_at"],"nullable_columns":{"expiry_date","next_billing_date"}},
    ...
    {"table":"tracks","file":"tracks.csv","columns":["id","owner_user_id","track_type","artist_id","album_id","track_number","title","lyrics","genre","audio_key","duration_sec","featured_artist_text","status","scheduled_publish_at","published_at","play_count","created_at","updated_at","deleted_at"],"nullable_columns":{"artist_id","album_id","track_number","lyrics","featured_artist_text","scheduled_publish_at","published_at","deleted_at"}},
    ...
    {"table":"track_applications","file":"track_applications.csv","columns":["id","requester_user_id","track_type","artist_id","album_id","track_number","title","lyrics","genre","audio_key","duration_sec","featured_artist_text","publish_delay_days","status","reviewed_by_admin_id","reviewed_at","rejection_reason","created_at","updated_at"],"nullable_columns":{"artist_id","album_id","track_number","lyrics","featured_artist_text","publish_delay_days","reviewed_by_admin_id","reviewed_at","rejection_reason"}},
]

INT_COLUMNS = {"active","album_id","amount","artist_id","balance","balance_after","cash_amount","cash_order_id","deleted","duration_sec","event_balance","id","notification_enabled","owner_user_id","play_count","played_duration","playlist_id","point_amount","point_order_id","position","publish_delay_days","requester_user_id","result_count","reviewed_by_admin_id","subscription_amount","target_id","total_balance","total_duration_sec","track_count","track_id","track_number","user_id","wallet_id"}
